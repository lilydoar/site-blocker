...
use clap::Parser;
use command::{AddResponse, Command, CommandResponse, RemoveResponse};
...
fn main() -> Result<(), std::io::Error> {
    ...
    let response = match Cli::parse().command {
        Command::List => CommandResponse::List(interactor.blocked_sites().to_vec()),
        Command::Add { site } => match interactor.blocked_sites().contains(&site) {
            true => CommandResponse::Add(AddResponse::AlreadyExists(site)),
            false => {
                interactor.add_site(&site)?;
                CommandResponse::Add(AddResponse::Added(site))
            }
        },
        Command::Remove { site } => match interactor.blocked_sites().contains(&site) {
            false => CommandResponse::Remove(RemoveResponse::NotFound(site)),
            true => {
                interactor.remove_site(&site)?;
                CommandResponse::Remove(RemoveResponse::Removed(site))
            }
        },
    };
